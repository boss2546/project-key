...
🧪 **Smoke test (in-process TestClient) ผลล่าสุด: 21/21 PASS** ✅
- **Frontend rendering** (5/5): `GET /`, `/legacy/{index.html, app.js, pricing.html, styles.css}` — ทุกหน้ามี "Personal Data Bank", zero "Project KEY"
- **App metadata** (2/2): `app.title = "Personal Data Bank"`, `app.version = "6.1.0"`
- **Backend imports** (1/1): main, llm, mcp_tools, billing, config, auth, database, profile — ทั้งหมด import สะอาด
- **Auth + MCP end-to-end** (4/4):
  - `POST /api/auth/register` → token ✓
  - `GET /api/auth/me` → email match (login flow intact) ✓
  - `GET /api/mcp/info` → version "6.1.0" ✓
  - `POST /mcp/{user-secret}` initialize → `serverInfo.name='personal-data-bank'` + `version='6.1.0'` ✓ (the real proof!)
- **Source-code KEEP invariants** (9/9):
  - llm.py: X-Title="Personal Data Bank", HTTP-Referer="https://project-key.fly.dev" (real URL)
  - config.py: APP_VERSION="6.1.0", projectkey.db filename kept
  - app.js: localStorage `projectkey_token`/`projectkey_user`/`projectkey_lang` kept; MCP templates changed to `personal-data-bank`; no `project-key` template left
  - fly.toml: `app = "project-key"` kept (Fly.io constraint)

> **Smoke test caught 1 bug ก่อน handoff:** WHY comment ของ `maybeShowRebrandNotice()` มี literal `"Project KEY"` ใน served JS file → fix ใน `312658e` (rebrand notice still preserved + เปลี่ยน comment เป็น "ชื่อเดิม")
