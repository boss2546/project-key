...
def t_mcp_info():
    ...
    return j.get("version", "") == "6.1.0" or "6.1.0" in str(j.get("version", ""))
run("GET /api/mcp/info -> version 6.1.0 + connector_url has secret", t_mcp_info)
...
def t_mcp_initialize():
    ...
    return s["name"] == "personal-data-bank" and s["version"] == "6.1.0"
run("POST /mcp/{secret} initialize -> serverInfo.name='personal-data-bank' + v6.1.0", t_mcp_initialize)
...
def t_brand_mcp_init():
    ...
    s = r.json().get("result", {}).get("serverInfo", {})
    return s.get("name") == "personal-data-bank" and s.get("version") == "6.1.0"
run("Brand: MCP serverInfo.name='personal-data-bank' + version='6.1.0'", t_brand_mcp_init)
...
run("BUMP: config.py APP_VERSION = '6.1.0'", lambda: 'APP_VERSION = "6.1.0"' in cfg)
