...
async def section_b_version():
    ...
    # v8.x update — accept any 7.x.y or 8.x.y version
    # Why so loose: dedupe feature shipped v7.1, but later versions (v7.2-v8.2)
    # piggyback APP_VERSION. Test should pass on any v7+/v8+.
    from backend.config import APP_VERSION
    expect_true(
        f"B.1 config.APP_VERSION starts with '7.' or '8.' (got '{APP_VERSION}')",
        APP_VERSION.startswith("7.") or APP_VERSION.startswith("8.") or APP_VERSION.startswith("9."),
    )

    # B.2: FastAPI app instance reads from APP_VERSION
    from backend.main import app
    expect_true(
        f"B.2 FastAPI app.version matches config (got '{app.version}')",
        app.version.startswith("7.") or app.version.startswith("8.") or app.version.startswith("9."),
    )
