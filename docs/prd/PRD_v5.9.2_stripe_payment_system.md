...
Webhook Secret: ยังไม่ได้ตั้ง — ต้องสร้างใน Stripe Dashboard > Webhooks
Starter Price ID: ยังไม่ได้สร้าง — ต้องสร้าง Product + Price ใน Stripe Dashboard
```
...
3. ❌ สร้าง Product "Personal AI Context — Starter" → ได้ `STRIPE_STARTER_PRODUCT_ID`
4. ❌ สร้าง Price 99 THB/month Recurring → ได้ `STRIPE_STARTER_PRICE_ID`
5. ❌ สร้าง Webhook Endpoint → ได้ `STRIPE_WEBHOOK_SECRET`
6. ❌ ตั้งค่า Customer Portal
...
STRIPE_STARTER_PRODUCT_ID ← ❌ รอสร้างใน Dashboard
STRIPE_STARTER_PRICE_ID   ← ❌ รอสร้างใน Dashboard
STRIPE_WEBHOOK_SECRET     ← ❌ รอสร้าง Webhook endpoint
APP_BASE_URL              ← ✅ ตั้งไว้ http://localhost:8000
```

---

## 6. User Flow — Upgrade to Starter

```text
User logs in
↓
User opens Pricing or Billing page
↓
User clicks “Get Starter” or “Upgrade to Starter”
↓
...
↓
User sees Starter status in Billing page
```
...
- Pricing section
- Billing / Plan page
- Upgrade modal ตอน Free user ชน limit
...
Button:
Upgrade to Starter — 99 บาท / เดือน
```
...
| `STRIPE_WEBHOOK_SECRET` | ❌ ว่างอยู่ | `.env` | ต้องสร้าง Webhook ก่อน |
| `STRIPE_STARTER_PRICE_ID` | ❌ ว่างอยู่ | `.env` | ต้องสร้าง Product/Price ก่อน |
...
1. Free user กด Get Starter แล้วไป Stripe Checkout ได้
2. ผู้ใช้จ่าย 99 บาท/เดือนได้ใน Stripe test mode
3. Stripe webhook ส่งกลับมาที่ระบบได้
4. ระบบ verify webhook signature ได้
5. หลังจ่ายสำเร็จ user กลายเป็น Starter
6. Billing page แสดง Current Plan: Starter
7. ผู้ใช้กด Manage Billing แล้วไป Stripe Customer Portal ได้
8. ผู้ใช้ยกเลิก subscription ได้ผ่าน Customer Portal
9. ระบบรับ cancel status และแสดง cancels at period end ได้
10. ระบบรับ payment failed event ได้
11. Executive plans ไม่มีปุ่มจ่ายเงินผ่าน Stripe Checkout
