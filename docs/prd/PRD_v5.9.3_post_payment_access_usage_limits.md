...
**Status:** ✅ Phase 2 Complete — Enforcement + Dashboard + Upgrade CTA + Downgrade Lock + Upgrade Unlock + Audit Log  
...
| Export Enforcement | `backend/main.py` | ✅ Done — `check_export_allowed` + `log_usage("export")` on download/share endpoints |
| Downgrade Logic | `backend/billing.py` | ✅ Done — `lock_excess_data` called on `subscription.deleted`; data preserved, access locked |
| Upgrade Unlock | `backend/billing.py` | ✅ Done — `unlock_data_for_plan` called on `checkout.session.completed`; restores Starter access |
| Webhook Payment Complete | `backend/billing.py` | ✅ Done — updates plan to starter_active on checkout.session.completed |
| AuditLog Table | `backend/database.py` | ✅ Done — `AuditLog` model + `log_audit()` helper; tracks plan_changed, data_unlocked, downgrade_completed |
| Locked Data Columns | `backend/database.py` | ✅ Done — `is_locked` + `locked_reason` on `File` and `ContextPack` with auto-migration |
