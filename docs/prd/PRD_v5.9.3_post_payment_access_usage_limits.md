...
**Status:** 🔄 In Progress — Phase 1 Backend Enforcement Done  
...
| Summary Enforcement | `backend/main.py` | ⏳ Next — log AI summary usage + check before generate |
| Export Enforcement | `backend/main.py` | ⏳ Next — log export usage + check before export |
| Refresh Enforcement | `backend/main.py` | ⏳ Next — log refresh usage + check before refresh |
| Dashboard Usage UI | `legacy-frontend/app.js` | ⏳ Next — display usage bars in profile/dashboard |
| Downgrade Logic | `backend/billing.py` | ⏳ Next — lock packs/files on downgrade |
| Upgrade Unlock | `backend/billing.py` | ⏳ Next — unlock packs/files on upgrade |
